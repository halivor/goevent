...
func (c *Consul) Get(key string) []byte {
...
func (c *Consul) Watch(prefix string) <-chan map[string][]byte {
	cb := make(chan map[string][]byte)
...
func (c *Consul) watch(key string) (map[string][]byte, error) {
	idx := c.getIdx(key)
	...
	dst := make(map[string][]byte, len(kvs))
	for _, kv := range kvs {
		dst[kv.Key] = kv.Value
	}
	c.Index.Store(key, meta.LastIndex)
...
func KVbEqual(src, dst map[string][]byte) bool {
	for k, vs := range src {
		if vd, ok := dst[k]; !ok || string(vd) != string(vs) {
			return false
		}
	}
	...
	for k, vd := range dst {
		if vs, ok := src[k]; !ok || string(vs) != string(vd) {
			return false
		}
	}
	return true
